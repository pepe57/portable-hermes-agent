...
class TestQuarantineBundleBinaryAssets:
    ...
    def test_quarantine_bundle_preserves_text_bytes(self, tmp_path):
        import tools.skills_hub as hub
        from tools.skills_guard import content_hash
        ...
        assert (q_path / "SKILL.md").read_bytes() == b"line one\nline two\n"
        assert bundle_content_hash(bundle) == content_hash(q_path)
