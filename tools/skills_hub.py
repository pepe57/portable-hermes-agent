    ...
    def fetch(self, identifier: str) -> Optional[SkillBundle]:
        ...
        for f in skill_dir.rglob("*"):
            if (
                ...
                # Bundle paths are an OS-independent wire format.  Using
                # ``str(Path)`` leaks backslashes on Windows, which breaks
                # lookups, nested binary assets, and content-hash symmetry.
...
def quarantine_bundle(bundle: SkillBundle) -> Path:
    ...
    for rel_path, file_content in validated_files:
        ...
        if isinstance(file_content, bytes):
            ...
            # Preserve the bundle's exact UTF-8 bytes on every OS.  Text-mode
            # writes translate LF to CRLF on Windows, making the installed
            # content hash differ from the downloaded bundle forever.
