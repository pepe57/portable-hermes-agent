...
def _update_via_zip(args, *, had_desktop_app_before_update: bool = False) -> bool:
    ...
    try:
        zip_path = os.path.join(tmp_dir, f"portable-hermes-agent-{branch}.zip")
...
def _get_upstream_url(git_cmd: list[str], cwd: Path) -> Optional[str]:
    """Get the URL of the upstream remote, or None if it is unavailable."""
    try:
        result = subprocess.run(
            git_cmd + ["remote", "get-url", "upstream"],
            cwd=cwd,
    ...
    *,
    ...
    if not has_upstream:
        ...
        if assume_yes or (
            ...
            print(f"  Add it later with: git remote add upstream {OFFICIAL_REPO_URL}")
            return False
