---
name: tts-server
description: Generate speech with the portable TTS Server.
version: "1.0"
author: aivrar
license: MIT
platforms: [windows]
metadata:
  hermes:
    tags: [tts, speech, voice, cloning, audio, gpu]
    category: extensions
---
...
Use this skill when the user wants to install or manage the portable TTS Server, load voice models, clone a voice, or generate speech or dialogue locally.
...
```python
import httpx
r = httpx.post("http://127.0.0.1:8200/api/tts/kokoro", json={
...
```python
r = httpx.post("http://127.0.0.1:8200/api/tts/xtts", json={
...
```python
r = httpx.post("http://127.0.0.1:8200/api/tts/dia", json={
...
```python
r = httpx.get("http://127.0.0.1:8200/api/tts/kokoro/voices")
r = httpx.get("http://127.0.0.1:8200/api/tts/xtts/voices")
r = httpx.get("http://127.0.0.1:8200/api/tts/bark/voices")
# ... (each model has /voices endpoint)
```

### List All Models & Status
```python
r = httpx.get("http://127.0.0.1:8200/api/models")
r = httpx.get("http://127.0.0.1:8200/api/models/status")
```

### Job Management
```python
r = httpx.get("http://127.0.0.1:8200/api/jobs")          # List all jobs
r = httpx.get("http://127.0.0.1:8200/api/jobs/{job_id}")  # Get job status + audio URL
```

### Load/Unload Models (GPU memory)
```python
httpx.post("http://127.0.0.1:8200/api/models/{model}/load")
httpx.post("http://127.0.0.1:8200/api/models/{model}/unload")
```

### Worker Management
```python
r = httpx.get("http://127.0.0.1:8200/api/workers")
httpx.post("http://127.0.0.1:8200/api/workers/spawn", json={"model": "kokoro", "device": "cuda:0"})
httpx.delete("http://127.0.0.1:8200/api/workers/{worker_id}")
```

### Whisper (Speech-to-Text)
```python
r = httpx.get("http://127.0.0.1:8200/api/whisper")
httpx.post("http://127.0.0.1:8200/api/whisper/base/load")
```

### GPU/Device Info
```python
r = httpx.get("http://127.0.0.1:8200/api/devices")
```
...
- Check health: `httpx.get("http://127.0.0.1:8200/health")`
